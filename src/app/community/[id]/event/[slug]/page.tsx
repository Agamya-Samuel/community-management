...
import { db } from "@/db";
import { events, onlineEventMetadata, onsiteEventMetadata, eventTags, communities } from "@/db/schema";
 ...
 * 
 * URL structure: /community/[id]/event/[slug]
 * 
 * Displays all information about an event within a community:
 ...
 * 
 * Fetches event data from the database using the event slug
 * Verifies that the event belongs to the specified community
 */
interface CommunityEventDetailPageProps {
  params: Promise<{ id: string; slug: string }> | { id: string; slug: string };
}

export default async function CommunityEventDetailPage({
  params,
}: CommunityEventDetailPageProps) {
  ...
  if (isNaN(communityId) || !eventSlug) {
    notFound();
  }
  ...
  // Fetch event details from database by slug
  // Also verify that the event belongs to this community
  ...
  // If event not found or doesn't belong to this community, return 404
